...
| and   {Δ} {φ ψ : SyntacticFormula L}     : φ ⋏ ψ ∈ Δ → Derivation2 T (insert φ Δ) →
  Derivation2 T (insert ψ Δ) → Derivation2 T Δ
| or    {Δ} {φ ψ : SyntacticFormula L}     : φ ⋎ ψ ∈ Δ → Derivation2 T (insert φ (insert ψ Δ)) →
  Derivation2 T Δ
| all   {Δ} {φ : SyntacticSemiformula L 1} : ∀' φ ∈ Δ →
  Derivation2 T (insert (Rewriting.free φ) (Δ.image Rewriting.shift)) → Derivation2 T Δ
| ex    {Δ} {φ : SyntacticSemiformula L 1} : ∃' φ ∈ Δ → (t : SyntacticTerm L) →
  Derivation2 T (insert (φ/[t]) Δ) → Derivation2 T Δ
...
/-- Imported declaration from the Incompleteness formalization. -/
def _root_.LO.FirstOrder.Derivation.toDerivation2 T : {Γ : Sequent L} → T ⟹ Γ → T ⊢₂ Γ.toFinset
  | _, Derivation.axL Δ R v            =>
    Derivation2.closed _ (Semiformula.rel R v) (by simp) (by simp)
  ...
  | _, @Derivation.and _ _ Δ φ ψ dp dq =>
    Derivation2.and (φ := φ) (ψ := ψ) (by simp)
      (Derivation2.wk (Derivation.toDerivation2 T dp) (by
        simp))
      (Derivation2.wk (Derivation.toDerivation2 T dq) (by
        simp))
...
/-- Imported declaration from the Incompleteness formalization. -/
noncomputable def _root_.LO.FirstOrder.Derivation2.toDerivation : {Γ :
    Finset (SyntacticFormula L)} → T ⊢₂ Γ → T ⟹ Γ.toList
  | _, Derivation2.closed Δ φ hp hn              =>
    Derivation.em (φ := φ) (by simp [hp]) (by simp [hn])
...
/-- Imported declaration from the Incompleteness formalization. -/
lemma provable_iff_derivable2 {φ} : T ⊢! φ ↔ T ⊢₂.! φ := derivable_iff_derivable2
